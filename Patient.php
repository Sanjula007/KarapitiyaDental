	...
	*/
	...
	public function followUpAdd(){
		...
		$form_data = $this->input->post();
		 $data = array(
		 'date'=>$pid,
		 'visit'=>$this->input->post('form_visit'),
		 'newFindings'=>$this->input->post('form_newFindings'),
		 'rxDone'=>$this->input->post('form_rxDone'),
		 'otherAppDate'=>$this->input->post('form_otherAppDate')
		 );
